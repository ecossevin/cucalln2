MODULE CUCALLN_MF_LOKI_MOD
  CONTAINS
  SUBROUTINE CUCALLN_MF_LOKI (PPLDARE, PPLRG, KSTEP, YDTHF, YDCST, YDERAD, YDML_PHY_SLIN, YDML_PHY_EC, YGFL, YDCHEM,  &
  ...
  & PCM1, PTENC, PSCAV, PSCAV0, YDSTACK_L)
    ...
    USE satur_LOKI_MOD, ONLY: satur_LOKI
    USE cumastrn_LOKI_MOD, ONLY: cumastrn_LOKI
    USE cuccdia_LOKI_MOD, ONLY: cuccdia_LOKI
    ...
    & ZENTHS(KLON, KLEV)
    REAL(KIND=JPRB) :: ZCONDFLL, ZCONDFLN
...
!$acc routine vector
    ...
    POINTER(IP_ZQP1, ZQP1)
    POINTER(IP_ZUP1, ZUP1)
    POINTER(IP_ZVP1, ZVP1)
    POINTER(IP_ZTU, ZTU)
    POINTER(IP_ZQU, ZQU)
    POINTER(IP_ZQSAT, ZQSAT)
    POINTER(IP_ZRAIN, ZRAIN)
    POINTER(IP_ZCP1, ZCP1)
    POINTER(IP_ZQEA, ZQEA)
    POINTER(IP_ZTEA, ZTEA)
    POINTER(IP_ZVEA, ZVEA)
    POINTER(IP_ZUEA, ZUEA)
    POINTER(IP_ZENTHD, ZENTHD)
    POINTER(IP_ZENTHS, ZENTHS)
    YLSTACK_L = YDSTACK_L
    IP_ZTP1 = YLSTACK_L
    ...
    IP_ZENTHS = YLSTACK_L
    YLSTACK_L = YLSTACK_L + ISHFT(ISHFT(KLON*KLEV*C_SIZEOF(REAL(1, kind=JPRB)) + 7, -3), 3)
!$acc data present( PDX, LDLAND, PTM1, PQM1, PUM1, PVM1, PLITOT, PVERVEL, PQHFL, PAHFS, PAPHM1, PAP, PAPH, PGEO, PGEOH, PGAW,  &
!$acc & PCUCONVCA, PGP2DSPP, PTENT, PTENQ, PTENU, PTENV, PTENTA, PTENQA, PARPRC, KTOPC, KBASEC, KTYPE, KCBOT, KCTOP, KBOTSC,  &
!$acc & LDCUM, LDSC, KCBOT_LIG, KCTOP_LIG, LDCUM_LIG, LDSHCV, PLCRIT_AER, PLU, PLUDE, PLUDELI, PSNDE, PMFU, PMFD, PLGLAC,  &
!$acc & PDIFCQ, PDIFCS, PFHPCL, PFHPCN, PFPLCL, PFPLCN, PLRAIN, PRSUD, PSTRCU, PSTRCV, PFCQLF, PFCQIF, PMFUDE_RATE, PMFDDE_RATE,  &
!$acc & PCAPE, PWU, PWMEAN, PVDISCU, PDISS, PCM1, PTENC, PSCAV, PSCAV0, YDTHF, YDCST, YDERAD, YDML_PHY_SLIN, YDML_PHY_EC, YGFL,  &
!$acc & YDCHEM, YDSPP_CONFIG, YDPERTPAR )
...
!$acc loop seq
    ...
    IFLAG = 1
    CALL SATUR_LOKI(YDTHF, YDCST, KIDIA, KFDIA, KLON, YDML_PHY_EC%YRECUMF%NJKT2, KLEV, YDML_PHY_SLIN%YREPHLI%LPHYLIN, PAP, ZTP1,  &
    & ZQSAT, IFLAG, YDSTACK_L=YLSTACK_L)
...
!$acc loop vector
    ...
    LLTDKMF = .true.
    CALL CUMASTRN_LOKI(PPLDARE, PPLRG, YDTHF, YDCST, YDML_PHY_SLIN, YDML_PHY_EC, YGFL, YDCHEM, YDSPP_CONFIG, YDPERTPAR, KIDIA,  &
    ...
    & PFPLCN, ZRAIN, PLRAIN, PRSUD, PMFU, PMFD, PLGLAC, PMFUDE_RATE, PMFDDE_RATE, PCAPE, PWU, PWMEAN, PVDISCU, PDISS, KTRAC,  &
    & ZCP1, PTENC, PSCAV, PSCAV0, YDSTACK_L=YLSTACK_L)
    !----------------------------------------------------------------------
    
    !*    3.0       CALL 'CUCCDIA' TO UPDATE CLOUD PARAMETERS FOR RADIATION
    !               -------------------------------------------------------
    
    CALL CUCCDIA_LOKI(YDERAD, YDML_PHY_SLIN%YREPHLI, YDML_PHY_EC%YREPHY, KIDIA, KFDIA, KLON, KLEV, KSTEP, KCBOT, KCTOP, LDCUM,  &
    & ZQU, PLU, PMFU, ZRAIN, PARPRC, KTOPC, KBASEC, YDSTACK_L=YLSTACK_L)
...
!$acc end data
  END SUBROUTINE CUCALLN_MF_LOKI
END MODULE CUCALLN_MF_LOKI_MOD
